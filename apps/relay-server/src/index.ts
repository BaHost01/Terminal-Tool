...
function getSession(hostId: string): Session {
...
wss.on('connection', (socket: WebSocket, req) => {
  ...
  socket.on('message', (data: Buffer) => {
    if (role === 'host') {
      let hostMsg: terminal.HostMessage;
      try {
      ...
      if (!authenticated) {
        ...
        if (hostMsg.authRequest) {
          try {
            const decoded: any = jwt.verify(hostMsg.authRequest.token || '', JWT_SECRET);
            if (decoded.role !== 'host') throw new Error('Invalid role');
            currentHostId = hostMsg.authRequest.hostId || decoded.hostId;
            authenticated = true;
            
            const session = getSession(currentHostId as string);
            if (session.hostSocket) {
               session.hostSocket.close(1008, 'Conflict');
            }
            session.hostSocket = socket;
      ...
      if (hostMsg.ptyOutput) {
        ...
        if (targetClientId) {
          ...
          if (target) sendServerMessage(target, { ptyOutput: hostMsg.ptyOutput });
        } else {
        ...
        if (targetClientId) {
           const target = session.clientSockets.get(targetClientId);
           if (target) sendServerMessage(target, { ptyExit: hostMsg.ptyExit });
        } else {
          for (const client of session.clientSockets.values()) {
      ...
      let clientMsg: terminal.ClientMessage;
      try {
      ...
      if (!authenticated) {
        if (clientMsg.registerHost) {
          ...
          if (!hostId) return sendErrorMessage(socket, 'hostId required');
        ...
        if (clientMsg.authRequest) {
           try {
            const decoded: any = jwt.verify(clientMsg.authRequest.token || '', JWT_SECRET);
            currentHostId = clientMsg.authRequest.hostId || decoded.hostId;
            authenticated = true;
            getSession(currentHostId as string).clientSockets.set(clientId, socket);
